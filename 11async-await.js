...
// 1. async 函数内的 await 后面跟 promise对象，也可以是原始类型值(但会被转成一个立即resolve的promise对象);
...
// 4. async函数返回值promise的状态变化, 碰到错误或return 语句就返回，错误可以被catch捕获
    {
        ...
        // 综上，如果想继续执行下面的代码，可以将可能出错的那部分代码用try{}catch(err){}包裹，或者在await Promise.reject('aiyou,chu cuo le!').catch(),以防阻塞后续代码执行
...
// 5. 并发执行 VS 继发执行
        {
            ...
            // 继发执行 --> 一般不会这样去做，要等到上面的请求返回才去做下次请求，太耗时.
            // map,forEach循环加了async还是会并发执行，要继发执行必须用for循环才行 
            {   
                function request(value){
                    return new Promise((res,rej)=>{
                        setTimeout(()=>{
...
// 6. 知道一下async函数的实现原理：它是generator和自动执行器封装成的一个函数。

// 7. 了解内容：异步遍历器接口，它调用next()方法后返回的是promise对象，把原来的{value:'',done:false}作为then方法的参数传入；
//    它被部署在对象的Symbol.asyncIterator属性上了。

// 8. for await ...of...用来遍历异步/同步的遍历器对象

// 9. 了解内容：异步generator函数 
        {
